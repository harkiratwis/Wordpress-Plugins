...
function special_image_gallery_count_images() {
	...
	$images1 = explode( ',', trim($images) );
	$number = count( array_filter($images1) );
	return $number;
}
// To show View Gallery button on frontpage
function post_special_gallery( $content ) {
	if ( is_single()) {
		if ( special_image_gallery_count_images() > 0) {
			global $post;
			$jscolor = get_option( 'js_color' );
			$post_txt = (special_image_gallery_count_images() > 1) ? ' Photos' : ' Photo';
			$custom_content = '<div class="sp_gallery"><a style="background-color:#'.$jscolor.';"  id="open-popup" href="#special-gallery-'.$post->ID.'" class="spl-gallery-link" data-class="post-gallery-lightbox"><div class="rel">View Gallery<br><em>'.special_image_gallery_count_images().' '.$post_txt.'</em></div></a></div>';
